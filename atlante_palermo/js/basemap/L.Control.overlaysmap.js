L.Control.Basemaps = L.Control.extend({
    ...
    onAdd: function (map) {
        this._map = map;
        var container = L.DomUtil.create('div', 'overlaysmap leaflet-control closed');
        ...
        this.options.overlaysmap.forEach(function(d, i){
            var basemapClass = 'overlaysmap';
            ...
            L.DomEvent.on(basemapNode, 'click', function() {
                ...
                if (d != this.basemap) {
                    ...
                    L.DomUtil.removeClass(document.getElementsByClassName('overlaysmap active')[0], 'active');
                    L.DomUtil.addClass(basemapNode, 'active');

                    var altIdx = (i+1) % this.options.overlaysmap.length;
                    L.DomUtil.removeClass(document.getElementsByClassName('overlaysmap alt')[0], 'alt');
                    L.DomUtil.addClass(document.getElementsByClassName('overlaysmap')[altIdx], 'alt');
                }
            }, this);
...
L.control.overlaysmap = function (options) {
  return new L.Control.overlaysmap(options);
};